<template>
    ...
    <div class="pl30 pr30 pt30 rel">
        ...
        <div class="cell flex ac mt20">
            <input type="text" v-model="account" :placeholder="$t('微信账号')" class="flex1 size28">
        </div>
...
const loadDetail = async () => {
    const res = await apiOtcPaymentDetail({ type: 'wechat' }) as unknown as WechatPayment
    if (!res?.id) return
...
const submit = async () => {
    ...
    if(!account.value)return message(t('请输入微信账号'))
